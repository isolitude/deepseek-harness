...
基于较早 release 的功能分支会随着上游推进而偏离。这里 `feat/llmpwa-workbench` 基于 `dsh-v0.1.6-alpha.2`（`ddefc45fbc`），它早于 0.1.7 的源码启动修复链。把它合并到某个固定的上游 commit 有落在缺乏后续修复的 base 上的风险；合并到任意 master commit 则没有稳定、可评审的目标。
...
具体来说，本次合并的目标是 `dsh-v0.1.7-alpha.2`，它解析为 commit `00102833df`，正好等于 `upstream/master`。解决全部冲突后，要验证目标修复确实存在于合并后的树中——这里 `packages/boot/app-boot/` 的 resolver 与 profile 文件与 release 尖端零差异一致——并确认分支自身的 commit 在合并后得以保留。

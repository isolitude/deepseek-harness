...
describe('dsh-base bundle', () => {
  it('declares a parseable patch list through the dsh.bundle.patch manifest field', () => {
    ...
    expect(rows.find(row => row.id === 'web')?.config).toMatchObject({
      searchProvider: 'easytransnote',
      fetchProvider: 'easytransnote',
    })
    expect(rows.find(row => row.id === 'web-easytransnote')).toBeDefined()
    ...
    expect(manifest.dependencies).toHaveProperty('@deepseek-ai/dsh-web-easytransnote')
  })

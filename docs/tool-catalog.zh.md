...
| `@deepseek-ai/dsh-tool-remote` | `remote_edit`、`remote_exec`、`remote_pull`、`remote_push`、`remote_read`、`remote_write` | `ctx.tools`、`ctx.remote`、`ctx.systemPrompt`、`configured credential at call time` | `tool/call`、`tool/result` | - | remote_* 工具是远程执行接缝（ctx.remote）的模型可见消费方。每次调用都向上解析调用方会话最近的 `.dsh/config.yml`，直到出现 `remote:` 块，在工具边界内物化认证，并委托给已挂载的后端（dsh-remote-ssh2）；配置缺失时显式报错。push/pull 默认拒绝覆盖。 |
...
对远程服务器上现有的 UTF-8 文件应用一次字面量文本替换。默认情况下 old_string 必须恰好出现一次；设置 replace_all 可替换每次出现。
...
在该 analysis 配置的远程服务器上运行一条 shell 命令。返回有界 stdout/stderr 与远程退出码；非零退出是报告的结果，而不是失败。
...
把远程文件下载到本地会话工作区（SFTP）。除非 overwrite 为 true，否则拒绝覆盖已存在的本地文件。
...
把本地文件上传到远程服务器（SFTP）。除非 overwrite 为 true，否则拒绝覆盖已存在的远程文件。
...
读取远程服务器上带行号的文本文件。用 offset 与 limit 对大文件分页。
...
在远程服务器上创建或完全替换 UTF-8 文本文件。写入是原子的；失败的写入不留下半成品文件。
